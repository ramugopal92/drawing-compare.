...
def _sheet_sections(doc_result) -> str:
    ...
    for idx, page in enumerate(doc_result.pages, start=1):
        ...
        # Both revisions side by side, marked in the same places: the old
        # sheet showing what was there, the new one showing what replaced
        # it. One annotated sheet tells a reviewer where to look; two tell
        # them what actually differs.
        panels: list[str] = []
        for caption, image in (
            ("Baseline revision (old)", result.overlay_image),
            ("Compared revision (new)", getattr(result, "new_overlay_image", None)),
        ):
            if image is None:
                continue
            ...
            panels.append(
                ...
                f'<div class="sub" style="margin:0 0 4px">{_esc(caption)}</div>'
                f'<a href="data:image/png;base64,{b64}" target="_blank" '
                f'title="Open full size in a new tab">'
                f'<img src="data:image/png;base64,{b64}" alt="{_esc(caption)} {title}"/></a>'
                f'</div>'
            )
        if panels:
            parts.append(
                ...
                + "".join(panels)
                + "</div>"
                + '<p class="sub" style="margin-top:4px">Click either sheet to '
                "open it full size in a new tab, where it can be zoomed.</p>"
            )
...
{_revision_summary_section(doc_result)}
...
This report supports engineering review and does not replace it.
<div style="margin-top:10px;padding-top:8px;border-top:1px solid var(--line)">
<b>Ramu Gopal</b> — CAD Automation | AI Systems Developer ·
<a href="https://thetechthinker.com/ramu-gopal/">thetechthinker.com</a>
</div></div>
</body></html>"""
